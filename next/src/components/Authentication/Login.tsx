...
const Login = () => {
    ...
    const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        Auth.login(formData.emailAddress, formData.password).then((r) => (r?.status === 200 ? router.push('/') : false))
    }
    ...
    const oauthOptions = [
        {
            name: 'Google',
            icon: faGoogle,
            url: `${Config.authUrl}/google`,
        },
        {
            name: 'Discord',
            icon: faDiscord,
            url: `${Config.authUrl}/discord`,
        },
        {
            name: 'GitHub',
            icon: faGithub,
            url: `${Config.authUrl}/github`,
        },
    ]
            ...
            <div className="_authForm">
                <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4">
                    ...
                    {oauthOptions.map((item, k) => (
                        <a key={k} href={item.url} className="relative py-2 px-4 rounded-lg border hover:border-gray-600" rel="noopener noreferrer" target="_blank">
                            <FontAwesomeIcon icon={item.icon} className="opacity-80 absolute left-4 top-0 bottom-0 my-auto w-5" />
                            <div className="text-center">Sign in with {item.name}</div>
                        </a>
                    ))}
                    <div className="my-1 border-t" />
                ...
                <Link to="/auth/register" className="absolute left-0 right-0 -bottom-12 text-center">
