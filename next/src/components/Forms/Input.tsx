...
const Input = (props: IProps) => {
    ...
    const handleOnBlur = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        if (e.target.value) {
            ...
            if (required && !e.target.value) setError('Required.')
            else setError('')
        }
    }
    ...
    return (
        <div className={`${className} relative`}>
            <div className={`_formField ${error ? 'is-invalid' : 'is-valid'}`}>
                ...
                {type === 'textarea' && <textarea name={name} value={value || ''} minLength={minimumValue} maxLength={maximumValue} placeholder={label} onChange={handleUpdate} onBlur={handleOnBlur} />}
                {type === 'select' && (
                    <select name={name} value={value} onChange={handleUpdate}>
                        {options &&
